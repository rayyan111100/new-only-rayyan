...
export default function DashboardToolbar({ folders, activeFolderId, activeTabId, onFolderChange, onTabChange, onNewFolder, onEditFolder, onDeleteFolder, onNewTab, onDeleteTab, onRenameTab }) {
  ...
  const [savedVizList, setSavedVizList] = useState([])
  ...
  const handleAddPanel = (vizId) => {
    if (!activeDashboard) return
    let title = 'New Panel', vizType = 'bar', vizConfig = null
    if (vizId) {
      const v = vizService.get(vizId)
      if (v) { title = v.name; vizType = v.type || 'bar'; vizConfig = v.config || {} }
    }
    const panel = { id: 'panel_' + Date.now(), title, vizType, vizId: vizId || null, vizConfig, x: 0, y: 0, data: null, lastUpdated: null }
    addPanel(panel)
    dashboardService.addPanel(activeDashboard.id, panel)
  }
    ...
    <>
      ...
      <div className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-[#1a1d27] rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm">
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button onClick={() => window.dispatchEvent(new CustomEvent('open-share'))} disabled={!activeDashboard} title="Share"
            className="p-1.5 rounded-md text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors disabled:opacity-40">
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="M8.59 13.51l6.83 3.98M15.41 6.51l-6.82 3.98"/></svg>
          </button>
          <button onClick={() => window.dispatchEvent(new CustomEvent('open-report'))} disabled={!activeDashboard} title="Report"
            className="p-1.5 rounded-md text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors disabled:opacity-40">
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
          </button>
        </div>
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button onClick={() => { setSavedVizList(vizService.list()); setShowSavedViz(true) }} disabled={!activeDashboard} title="Add from Saved"
            className="p-1.5 rounded-md text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors disabled:opacity-40">
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/></svg>
          </button>
      ...
      {/* Saved Viz Modal */}
      {showSavedViz && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setShowSavedViz(false)}>
          <div className="bg-white dark:bg-[#1a1d27] rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-2xl w-full max-w-lg mx-3 max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-zinc-200 dark:border-zinc-700">
              <div><h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100">Add from Saved</h3><p className="text-[10px] text-zinc-400 mt-0.5">Select a visualization to add as a panel</p></div>
              <button onClick={() => setShowSavedViz(false)} className="p-1.5 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400"><svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
            </div>
            <div className="px-5 py-3 border-b border-zinc-200 dark:border-zinc-700">
              <input type="text" value={vizSearch} onChange={e => setVizSearch(e.target.value)} placeholder="Search saved visualizations..." className="w-full px-3 py-2 text-[11px] bg-zinc-50 dark:bg-zinc-800/60 rounded-xl outline-none text-zinc-800 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-700" autoFocus />
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-1">
              {savedVizList.filter(v => !vizSearch || v.name.toLowerCase().includes(vizSearch.toLowerCase())).length === 0 ? (
                <div className="text-center py-8 text-xs text-zinc-400">No saved visualizations found</div>
              ) : savedVizList.filter(v => !vizSearch || v.name.toLowerCase().includes(vizSearch.toLowerCase())).map(v => {
                const meta = { area: '📈', 'bar-vertical': '📊', 'bar-horizontal': '📊', line: '📉', pie: '🥧', heatmap: '🗺️', gauge: '🎯', goal: '🏁', metric: '🔢', table: '📋' }
                return (
                  <button key={v.id} onClick={() => { handleAddPanel(v.id); setShowSavedViz(false) }}
                    ...
                    <span className="text-xl shrink-0">{meta[v.type] || '📊'}</span>
                    <div className="min-w-0 flex-1">
                      <div className="text-[11px] font-semibold text-zinc-700 dark:text-zinc-200 truncate">{v.name}</div>
                      <div className="text-[9px] text-zinc-400">{v.type} · {new Date(v.updatedAt).toLocaleDateString()}</div>
                    </div>
