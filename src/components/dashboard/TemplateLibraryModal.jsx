...
import { getTemplateList, getTemplate, createFromTemplate, TEMPLATE_CATEGORIES } from './dashboardTemplates'

const TEMPLATE_COLORS = {
  soc: '#8b5cf6', executive: '#10b981', security: '#ef4444',
  platform: '#06b6d4', network: '#f59e0b'
}
...
export default function TemplateLibraryModal({ open, onClose, onUseTemplate }) {
  ...
  // Fetch available agents for filter
  useEffect(() => {
  ...
  const handleConfirmWithFilter = () => {
    ...
    if (dash) {
      // Set selected agents as global filters
        ...
        <div className="bg-white dark:bg-[#1a1d27] rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-2xl w-full max-w-lg mx-3" onClick={e => e.stopPropagation()}>
          <div className="px-5 py-3.5 border-b border-zinc-200 dark:border-zinc-700">
            <div className="flex items-center gap-2">
              <span className="w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold text-white shrink-0" style={{backgroundColor: tpl ? (TEMPLATE_COLORS[tpl.category] || '#8b5cf6') : '#8b5cf6'}}>{tpl?.name ? tpl.name[0] : '?'}</span>
              <div>
                <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100">Filter by Agent</h3>
                <p className="text-[10px] text-zinc-400 mt-0.5">Select agents to filter "{tpl?.name}" dashboard (optional)</p>
              </div>
            </div>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onClose}>
      <div className="bg-white dark:bg-[#1a1d27] rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-2xl w-full max-w-4xl mx-3 max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-3 border-b border-zinc-200 dark:border-zinc-700 space-y-3">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search templates..."
              className="w-full pl-9 pr-3 py-2 text-[11px] bg-zinc-50 dark:bg-zinc-800/60 rounded-xl outline-none border border-zinc-200 dark:border-zinc-700" />
          </div>
          <div className="flex flex-wrap gap-1.5">
            <button onClick={() => setCategory(null)} className={`px-3 py-1 text-[10px] font-medium rounded-lg border transition-colors ${!category ? 'bg-[#8b5cf6] text-white border-[#8b5cf6]' : 'border-zinc-200 dark:border-zinc-700 text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800'}`}>All</button>
            {TEMPLATE_CATEGORIES.map(c => (
              <button key={c.id} onClick={() => setCategory(category === c.id ? null : c.id)}
                className={`px-3 py-1 text-[10px] font-medium rounded-lg border transition-colors ${category === c.id ? 'bg-[#8b5cf6] text-white border-[#8b5cf6]' : 'border-zinc-200 dark:border-zinc-700 text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800'}`}>{c.label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {previewTpl ? (
            <div>
              <button onClick={() => setPreviewId(null)} className="text-[10px] text-[#8b5cf6] hover:underline mb-3 flex items-center gap-1">
              ...
              <div className="bg-zinc-50 dark:bg-zinc-800/40 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700">
                <div className="flex items-center gap-3 mb-3">
                  <span className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0" style={{backgroundColor: TEMPLATE_COLORS[previewTpl.category] || '#8b5cf6'}}>{previewTpl.name[0]}</span>
                  <div>
                  ...
                  <span className="text-[9px] px-2 py-1 rounded-md bg-[#8b5cf6]/10 text-[#8b5cf6]">{previewTpl.panels.length} panels</span>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {previewTpl.panels.map((p, i) => (
                    <div key={i} className="flex items-center gap-2 px-2.5 py-2 rounded-lg bg-white dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700">
                      <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: p.type === 'metric' ? '#10b981' : p.type === 'pie' ? '#8b5cf6' : p.type === 'table' ? '#6b7280' : '#EF843C' }} />
              ...
              <button onClick={() => { handleUseTemplate(previewId) }}
                className="mt-4 w-full py-2.5 text-[11px] font-semibold rounded-xl bg-gradient-to-r from-[#8b5cf6] to-[#7c3aed] text-white hover:from-[#7c3aed] hover:to-[#6d28d9] transition-all">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {filtered.map(t => (
                <div key={t.id} className="bg-white dark:bg-zinc-800/40 rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 hover:border-[#8b5cf6]/40 hover:shadow-sm transition-all group">
                  <div className="flex items-start gap-3 mb-3">
                    <span className="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold text-white shrink-0" style={{backgroundColor: TEMPLATE_COLORS[t.category] || '#8b5cf6'}}>{t.name[0]}</span>
                    <div className="min-w-0">
                      <h4 className="text-[11px] font-bold text-zinc-800 dark:text-zinc-100 truncate">{t.name}</h4>
                      <p className="text-[9px] text-zinc-400 dark:text-zinc-500 line-clamp-2">{t.description}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 text-[9px] text-zinc-400 mb-3">
                    <span className="px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800">{t.category}</span>
                    <span>{t.panelCount} panels</span>
                  </div>
                  <div className="flex gap-1.5">
                    <button onClick={() => setPreviewId(t.id)} className="flex-1 px-2 py-1.5 text-[9px] font-semibold rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">Preview</button>
                    <button onClick={() => handleUseTemplate(t.id)}
                      className="flex-1 px-2 py-1.5 text-[9px] font-semibold rounded-lg bg-[#8b5cf6] text-white hover:bg-[#7c3aed] transition-colors">Use</button>
                  </div>
                </div>
              ))}
            </div>
