...
export default function FimTab() {
  ...
  const fetchEvents = useCallback(async (searchQ) => {
    ...
    setError(null)
    try {
      const base = { index: 'unishield360-alerts-4.x-*', start_date: 'now-7d', end_date: 'now' }
      let query
      if (searchQ) {
        query = `(rule.groups:*fim* OR rule.groups:*file_integrity* OR rule.groups:*syscheck*) AND (${searchQ})`
      } else {
        query = 'rule.groups:*fim* OR rule.groups:*file_integrity* OR rule.groups:*syscheck*'
      }
  ...
  const syscheckPath = (ev) => {
    const s = ev.syscheck || ev.data?.syscheck || {}
    return s.path || ev.data?.file || ev.data?.path || '-'
  }
    ...
    <div className="p-4 h-full flex flex-col bg-[#f8f9fc] dark:bg-[#0e0f14] gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1 className="text-sm font-semibold text-[#1a1c23] dark:text-[#e4e6eb]">File Integrity Monitoring</h1>
        ...
        <form onSubmit={handleSearch} className="flex gap-1">
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Filter FIM events..." className="ginput text-[11px] px-2 py-1 w-48" />
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-[#9ca3af] text-xs gap-2">
          <svg className="w-8 h-8 opacity-40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          No FIM events found
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-[#e5e7eb]/50 dark:divide-[#2d3140]/50">
                {events.map((ev, i) => (
                  <tr key={ev._id || i} onClick={() => setSelected(selected === i ? null : i)}
                    ...
                    <td className="py-1 px-2 text-[#EF843C] font-mono text-[10px] truncate max-w-[200px]">{syscheckPath(ev)}</td>
                    <td className="py-1 px-2"><SevBadge level={ev.rule?.level || 0} /></td>
                    <td className="py-1 px-2 text-soc-stext dark:text-soc-darkstext truncate max-w-[250px]">{ev.rule?.description || ev.full_log?.slice(0, 80) || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
