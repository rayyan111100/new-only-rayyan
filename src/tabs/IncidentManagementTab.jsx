    ...
    <div className="p-4 h-full flex flex-col bg-[#f8f9fc] dark:bg-[#0e0f14] gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1 className="text-sm font-semibold text-[#1a1c23] dark:text-[#e4e6eb]">Incident Management</h1>
          {!loading && <span className="text-[10px] text-[#9ca3af] bg-[#f3f4f6] dark:bg-[#2d3140] px-1.5 py-0.5 rounded-full">{total} incidents</span>}
        </div>
        <div className="flex items-center gap-2">
          ...
          <form onSubmit={handleSearch} className="flex gap-1">
            <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search incidents..." className="ginput text-[11px] px-2 py-1 w-48" />
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-[#9ca3af] text-xs gap-2">
          <svg className="w-8 h-8 opacity-40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
          No incidents found
        </div>
      ) : (
            ...
            {selected !== null && events[selected] && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] font-semibold uppercase text-[#9ca3af] tracking-wider">Incident Details</span>
