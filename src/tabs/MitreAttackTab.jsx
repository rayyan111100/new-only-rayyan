      ...
      <table className="w-full text-xs border-collapse">
        <thead>
          <tr className="text-[10px] text-[#8b949e] font-bold uppercase tracking-wide">
            {cols.map(c => (
              <th key={c.key} className={`text-left py-2 px-3 border-b border-[#d0d7de] dark:border-[#30363d] ${c.sort ? 'cursor-pointer hover:text-[#EF843C]' : ''}`}
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
      ...
      {view === 'dashboard' && (
        <div className="space-y-3">
          <div className="bg-white dark:bg-[#161b22] border border-[#d0d7de] dark:border-[#30363d] rounded-xl p-3 shadow-sm">
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="flex-1 min-w-0">
                  <table className="w-full text-[11px] border-collapse">
                    ...
                    <tfoot>
                      <tr className="border-t border-[#d0d7de] dark:border-[#30363d]">
      ...
      {view === 'intelligence' && (
        <div className="flex gap-3 h-[calc(100vh-220px)]">
          ...
          <div className="flex-1 bg-white dark:bg-[#161b22] border border-[#d0d7de] dark:border-[#30363d] rounded-xl overflow-hidden flex flex-col shadow-sm">
            <div className="p-2 border-b border-[#d0d7de] dark:border-[#30363d]">
              <div className="relative">
                ...
                <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} placeholder="Search by name, id..." className="w-full pl-8 pr-3 py-1.5 text-xs bg-[#f6f8fa] dark:bg-[#0d1117] border border-[#d0d7de] dark:border-[#30363d] rounded-lg text-[#1a1c23] dark:text-[#e4e6eb] outline-none focus:border-[#EF843C]" />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto">{renderTable()}</div>
            <div className="flex items-center justify-between px-3 py-2 border-t border-[#d0d7de] dark:border-[#30363d] bg-[#f6f8fa] dark:bg-[#0d1117]">
              <span className="text-[10px] text-[#8b949e]">{sortedData.length} items</span>
              <div className="flex items-center gap-1">
                <button onClick={() => setPage(1)} disabled={pg === 1} className="px-1.5 py-0.5 text-[10px] border border-[#d0d7de] dark:border-[#30363d] rounded disabled:opacity-30 bg-white dark:bg-[#161b22]"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="11 17 6 12 11 7"/><polyline points="18 17 13 12 18 7"/></svg></button>
                <button onClick={() => setPage(pg - 1)} disabled={pg === 1} className="px-1.5 py-0.5 text-[10px] border border-[#d0d7de] dark:border-[#30363d] rounded disabled:opacity-30 bg-white dark:bg-[#161b22]"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg></button>
                <span className="text-[10px] text-[#8b949e] px-1">{pg}/{totalPages}</span>
                <button onClick={() => setPage(pg + 1)} disabled={pg === totalPages} className="px-1.5 py-0.5 text-[10px] border border-[#d0d7de] dark:border-[#30363d] rounded disabled:opacity-30 bg-white dark:bg-[#161b22]"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg></button>
                <button onClick={() => setPage(totalPages)} disabled={pg === totalPages} className="px-1.5 py-0.5 text-[10px] border border-[#d0d7de] dark:border-[#30363d] rounded disabled:opacity-30 bg-white dark:bg-[#161b22]"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="13 17 18 12 13 7"/><polyline points="6 17 11 12 6 7"/></svg></button>
              </div>
            </div>
          </div>
          <div className="w-72 shrink-0 bg-white dark:bg-[#161b22] border border-[#d0d7de] dark:border-[#30363d] rounded-xl overflow-hidden flex flex-col shadow-sm">
            <div className="px-3 py-2 border-b border-[#d0d7de] dark:border-[#30363d] bg-[#f6f8fa] dark:bg-[#0d1117]">
      ...
      {view === 'framework' && (
        <div className="bg-white dark:bg-[#161b22] border border-[#d0d7de] dark:border-[#30363d] rounded-xl p-3 shadow-sm overflow-x-auto">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-3 text-[10px] text-[#8b949e]">
              <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded bg-[#f6f8fa] dark:bg-[#0d1117] border border-[#d0d7de] dark:border-[#30363d]" />None</span>
            ...
            {(mitreKnowledge.tactics || []).sort((a, b) => a.order - b.order).map(tac => {
              ...
              return (
                <div key={tac.id || tac.name} className="w-36 shrink-0">
                  <div className="bg-[#f6f8fa] dark:bg-[#0d1117] border border-[#d0d7de] dark:border-[#30363d] rounded-t-lg px-2 py-1.5 text-center">
                  ...
                  {cells.length > 0 ? cells.map(t => (
                    <div key={t.id || t.name} onClick={() => { setView('intelligence'); setIntelSection('techniques'); setTimeout(() => { setSearch(t.name.split(' ')[0]) }, 100) }}
                      className={`border border-[#d0d7de] dark:border-[#30363d] border-t-0 px-2 py-1 text-[10px] text-[#1a1c23] dark:text-[#e4e6eb] cursor-pointer hover:bg-[#f0f2f4] dark:hover:bg-[#21262d] transition-colors ${heatClass(t.subCount || 0)}`}>
                      {t.name}
                      {(t.subCount || 0) > 0 && <span className="ml-1 text-[8px] text-[#8b949e] font-bold">{t.subCount}</span>}
                    </div>
                  )) : <div className="border border-[#d0d7de] dark:border-[#30363d] border-t-0 px-2 py-4 text-center text-[8px] text-[#8b949e]">No techniques</div>}
                </div>
              )
            })}
          </div>
      ...
      {view === 'events' && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => { fetchEvents() }} className="flex items-center gap-1 px-2 py-1 text-[10px] font-bold border border-[#d0d7de] dark:border-[#30363d] rounded-lg hover:border-[#EF843C] hover:text-[#EF843C]">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => { setEvFilterSev(null); setEvFilterTac(null); setEvFilterAgent(null) }}
              className={`px-2.5 py-1 text-[10px] font-bold rounded-lg border transition-all ${activeFilters ? 'bg-[#EF843C]/10 border-[#EF843C]/40 text-[#EF843C]' : 'text-[#8b949e] border-[#d0d7de] dark:border-[#30363d]'}`}>
              {activeFilters ? 'Clear filters' : 'All events'}
            </button>
          </div>

          {activeFilters && (
            <div className="flex items-center gap-1.5 flex-wrap px-2 py-1.5 bg-[#f6f8fa] dark:bg-[#0d1117] rounded-lg border border-[#d0d7de] dark:border-[#30363d]">
          ...
          <div className="bg-white dark:bg-[#161b22] border border-[#d0d7de] dark:border-[#30363d] rounded-xl overflow-hidden shadow-sm">
            {evFiltered.length > 0 ? (
                ...
                <table className="w-full text-xs border-collapse">
                  <thead>
                    <tr className="text-[10px] text-[#8b949e] font-bold uppercase tracking-wide bg-[#f6f8fa] dark:bg-[#0d1117]">
                      <th className="text-left py-2 px-3 border-b border-[#d0d7de] dark:border-[#30363d]">Timestamp</th>
                      <th className="text-left py-2 px-3 border-b border-[#d0d7de] dark:border-[#30363d]">Technique</th>
                      <th className="text-left py-2 px-3 border-b border-[#d0d7de] dark:border-[#30363d]">Tactic</th>
                      <th className="text-left py-2 px-3 border-b border-[#d0d7de] dark:border-[#30363d]">Agent</th>
                      <th className="text-left py-2 px-3 border-b border-[#d0d7de] dark:border-[#30363d]">Severity</th>
                      <th className="text-left py-2 px-3 border-b border-[#d0d7de] dark:border-[#30363d]">Rule</th>
                    </tr>
                  </thead>
                  <tbody>
                    {evFiltered.map((e, i) => (
              ...
              <div className="py-8 text-center">
                ...
                {activeFilters && <button onClick={() => { setEvFilterSev(null); setEvFilterTac(null); setEvFilterAgent(null) }} className="mt-2 px-3 py-1 text-xs font-bold border border-[#d0d7de] dark:border-[#30363d] rounded-lg hover:border-[#EF843C] hover:text-[#EF843C]">Clear filters</button>}
              </div>
            )}
          </div>
        </div>
