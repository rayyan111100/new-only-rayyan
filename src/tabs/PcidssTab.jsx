...
const LOG_DATA = [
  { time: '2025-05-24 12:15:34', agent: 'suyash-window', rule: '550', ctrl: '11.5', desc: 'Integrity checksum changed.', sev: 'High', event: 'modified', file: 'c:\\users\\smart resource\\...\\vulndetectionlab.jsx', groups: 'syscheck, syscheck_entry_modified' },
  { time: '2025-05-24 12:13:22', agent: 'WIN-DC01', rule: '554', ctrl: '6.4.2', desc: 'File added to system.', sev: 'Medium', event: 'added', file: 'c:\\windows\\system32\\drivers\\etc\\hosts', groups: 'syscheck, syscheck_file' },
  { time: '2025-05-24 12:11:09', agent: 'SRV-DB01', rule: '553', ctrl: '10.5.5', desc: 'File deleted from system.', sev: 'Medium', event: 'deleted', file: 'c:\\users\\smart resource\\...\\config.js', groups: 'syscheck, syscheck_file' },
  { time: '2025-05-24 12:10:02', agent: 'linux-server01', rule: '571', ctrl: '8.2.3', desc: 'User login failed.', sev: 'High', event: 'authentication_failed', file: 'sshd', groups: 'syslog, authentication_failed' },
  { time: '2025-05-24 12:08:47', agent: 'web-server02', rule: '562', ctrl: '3.4.1', desc: 'User authentication success.', sev: 'Low', event: 'authentication_success', file: 'nginx', groups: 'syslog, authentication_success' },
  { time: '2025-05-24 12:06:30', agent: 'suyash-window', rule: '550', ctrl: '11.5', desc: 'Integrity checksum changed.', sev: 'High', event: 'modified', file: 'c:\\program files\\pciapp\\bin\\agent.exe', groups: 'syscheck, syscheck_entry_modified' },
  { time: '2025-05-24 12:04:15', agent: 'WIN-DC01', rule: '554', ctrl: '6.4.2', desc: 'New file in monitored dir.', sev: 'Medium', event: 'added', file: 'c:\\inetpub\\wwwroot\\upload\\shell.php', groups: 'syscheck, syscheck_file' },
  { time: '2025-05-24 12:02:50', agent: 'SRV-DB01', rule: '550', ctrl: '11.5', desc: 'Integrity checksum changed.', sev: 'Critical', event: 'modified', file: '/etc/postgresql/pg_hba.conf', groups: 'syscheck, syscheck_entry_modified' },
  { time: '2025-05-24 12:00:12', agent: 'linux-server01', rule: '571', ctrl: '8.2.3', desc: 'Multiple login failures.', sev: 'High', event: 'authentication_failed', file: 'sshd', groups: 'syslog, authentication_failed' },
  { time: '2025-05-24 11:58:44', agent: 'web-server02', rule: '553', ctrl: '10.5.5', desc: 'Log file cleared.', sev: 'Critical', event: 'deleted', file: '/var/log/apache2/access.log', groups: 'syscheck, syscheck_file' },
]
...
export default function PcidssTab() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const tp = timeParams()
      const d = await api('compliance', { index: 'unishield360-alerts-4.x-*', start_date: tp.start_date, end_date: tp.end_date, framework: 'PCI-DSS' })
      setData({
        count24: d.count24 || 0,
        count7d: d.count7d || 0,
        severity: d.severity || {},
  ...
  const modalContent = () => {
    ...
    const md = {
      'm-events': {
        t: 'PCI-DSS Events',
        b: `Total PCI-DSS events: ${totalEvents.toLocaleString()}. Across 24 monitored assets this week. Top requirement: 11.5 (Integrity Controls).`
      },
      'm-crit': {
        t: 'Critical Violations',
        b: `Critical violations: ${data?.severity?.Critical || 0}. These require immediate remediation per PCI-DSS requirements.`
      }
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={closeModal}>
          <div className="bg-white dark:bg-[#161b22] border border-[#d0d7de] dark:border-[#30363d] rounded-xl p-5 w-[500px] max-h-[72vh] overflow-y-auto shadow-lg" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-bold text-[#1f2328] dark:text-[#f0f6fc]">Log Entry — {l.agent} (Rule {l.rule})</span>
            ...
            <div className="grid grid-cols-2 gap-2.5 mb-3">
              {[
                ...
                { label: 'Control', val: l.ctrl, cls: 'text-[#a371f7] font-semibold' },
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15 }} className="p-3">
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="text-[11px] text-[#8b949e]">Home / <span className="text-[#e8681a] cursor-pointer" onClick={() => openModal('go-overview')}>Compliance Management</span> / PCI-DSS</div>
      ...
      <div className="grid grid-cols-6 gap-2.5 mb-3">
        {[
          { key: 'm-events', label: 'PCI-DSS Events', val: totalEvents.toLocaleString(), change: '20%', up: true, icon: 'certificate', iconBg: '#a371f71a', iconColor: '#a371f7' },
          ...
          { key: 'm-controls', label: 'Controls Triggered', val: PCI_CONTROLS.length, sub: 'Unique PCI-DSS controls', icon: 'list-check', iconBg: '#3fb95018', iconColor: '#3fb950' },
            ...
            return (
              <div key={c.req} onClick={() => openModal('ctrl-' + c.req)}
                ...
                <div className="flex-1 h-2 bg-[#d0d7de] dark:bg-[#30363d] rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${pct}%`, background: 'linear-gradient(90deg,#7c3aed,#a371f7)' }} />
                </div>
            ...
            {data?.timeline?.length > 0 && (
              <ResponsiveContainer width="100%" height={150}>
                <AreaChart data={data.timeline}>
                  <defs><linearGradient id="pcidssGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#a371f7" stopOpacity={0.12} /><stop offset="95%" stopColor="#a371f7" stopOpacity={0} /></linearGradient></defs>
                  ...
                  <Tooltip content={<CustomTip />} />
                  <Area type="monotone" dataKey="count" stroke="#a371f7" fill="url(#pcidssGrad)" strokeWidth={2.5} dot={{ r: 3, fill: '#a371f7', stroke: isDark ? '#161b22' : '#ffffff', strokeWidth: 2 }} />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
              ...
              {PCI_CONTROLS.map((c, i) => {
                ...
                return (
                  <tr key={c.req} onClick={() => openModal('ctrl-' + c.req)} className="cursor-pointer hover:bg-[#f0f2f4] dark:hover:bg-[#21262d]">
                    ...
                    <td className="py-1 px-2 border-b border-[#f0f2f4] dark:border-[#21262d] text-[#a371f7] font-semibold">{c.req}</td>
      ...
      {/* Event Logs */}
      <div className="mb-3">
        <div className="text-sm font-bold text-[#1f2328] dark:text-[#f0f6fc] mb-2.5 tracking-tight">PCI-DSS Event Logs</div>
        <table className="w-full text-[10px] border-collapse table-fixed">
          <colgroup>
            <col style={{ width: '110px' }} /><col style={{ width: '100px' }} /><col style={{ width: '55px' }} />
            <col style={{ width: '68px' }} /><col style={{ width: '145px' }} /><col style={{ width: '90px' }} />
            <col style={{ width: '100px' }} /><col style={{ width: '160px' }} /><col style={{ width: '165px' }} />
          </colgroup>
          <thead>
          ...
          <tbody>
            {LOG_DATA.slice((logPage - 1) * LOG_PAGE_SIZE, logPage * LOG_PAGE_SIZE).map((l, i) => (
              <tr key={i} onClick={() => openModal('log-' + ((logPage - 1) * LOG_PAGE_SIZE + i))}
                ...
                <td className="py-1.5 px-2 border-b border-[#f0f2f4] dark:border-[#21262d] font-semibold text-[#a371f7]">{l.ctrl}</td>
        ...
        <div className="flex items-center justify-between mt-2.5 flex-wrap gap-2">
          ...
          <div className="flex items-center gap-1 text-[11px] text-[#8b949e]">
            <span className="mr-1">{(logPage - 1) * LOG_PAGE_SIZE + 1}-{Math.min(logPage * LOG_PAGE_SIZE, LOG_DATA.length)} of 348</span>
            ...
            <button onClick={() => setLogPage(70)}
              className="bg-transparent border border-[#d0d7de] dark:border-[#30363d] text-[#36454f] dark:text-[#c9d1d9] px-2 py-0.5 rounded text-[11px] min-w-[28px] hover:bg-[#f0f2f4] dark:hover:bg-[#21262d] hover:border-[#e8681a] transition-all">70</button>
