...
export default function PcidssTab() {
  ...
  const { data, loading, error, toSev, refresh } = useCompliance('PCI-DSS')
  ...
  const toLogEntry = (r) => ({
    ...
    ctrl: r.rule?.pci_dss || r.control || r.pci_requirement || '--'
  })
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15 }} className="p-3">
      ...
      <div className="grid grid-cols-6 gap-2.5 mb-3">
        {[
          ...
          { key: 'm-controls', label: 'Controls Triggered', val: data?.topRules?.length || '--', sub: 'Unique PCI-DSS controls', icon: 'list-check', iconBg: '#3fb95018', iconColor: '#3fb950' },
