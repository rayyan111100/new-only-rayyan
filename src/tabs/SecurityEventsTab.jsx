...
export default function SecurityEventsTab() {
  ...
  const fetchEvents = useCallback(async (searchQ) => {
    ...
    try {
      ...
      const query = searchQ || 'rule.groups:*security*'
      ...
      for (const b of sevBuckets) {
        const s = getSeverity(b.key)
        const cnt = b.doc_count || 0
        if (s === 'critical') c += cnt
        else if (s === 'high') h += cnt
        else if (s === 'medium') m += cnt
        else l += cnt
      }
    ...
    <div className="p-4 h-full flex flex-col bg-[#f8f9fc] dark:bg-[#0e0f14] gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1 className="text-sm font-semibold text-[#1a1c23] dark:text-[#e4e6eb]">Security Events</h1>
        ...
        <form onSubmit={handleSearch} className="flex gap-1">
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Filter events..." className="ginput text-[11px] px-2 py-1 w-48" />
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-[#9ca3af] text-xs gap-2">
          <svg className="w-8 h-8 opacity-40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
              ...
              <tbody className="divide-y divide-[#e5e7eb]/50 dark:divide-[#2d3140]/50">
                {events.map((ev, i) => (
                  <tr key={ev._id || i} onClick={() => setSelected(selected === i ? null : i)}
                    ...
                    <td className="py-1 px-2 text-soc-stext dark:text-soc-darkstext">{ev.rule?.id || '-'}</td>
