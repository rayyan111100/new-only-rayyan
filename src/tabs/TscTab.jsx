...
export default function TscTab() {
  ...
  const loadMoreLogs = useCallback(async () => {
    ...
    try {
      ...
      const res = await api('search', { index: 'unishield360-alerts-*', start_date: sd, end_date: ed, q: '_exists_:rule.tsc', limit: 10000, offset, sort: '@timestamp', order: 'desc' })
      const results = res.results || []
      const mapped = results.map(r => {
        const entry = toLogEntry(r)
        const tsc = r.rule?.tsc
        entry.ctrl = Array.isArray(tsc) ? tsc.join(', ') : (tsc || entry.ctrl || '--')
      ...
      <div className="grid grid-cols-[1.1fr_0.85fr_1.05fr] gap-2.5 mb-2.5">
        ...
        <div className="bg-white dark:bg-[#16181f] border border-[#e5e7eb] dark:border-[#2d3140] rounded-xl p-3 shadow-lg transition-all duration-300 hover:-translate-y-[2px] dark:hover:shadow-[0_8px_30px_rgba(232,104,26,0.12)] hover:border-[#e8681a]/30 dark:hover:border-[#e8681a]/40">
          ...
          {topControlsByCount.slice(0, 5).map(c => {
            ...
            return (
              <div key={c.ctrl} className="flex items-center gap-2 mb-1.5 py-1 px-1 rounded hover:bg-[#f0f2f4] dark:hover:bg-[#21262d] text-[11px]">
                <InlineFilter field="control" value={c.ctrl}
                  onInclude={() => setInclude('control', c.ctrl)}
                  onExclude={() => setExclude('control', c.ctrl)}
                  isIncluded={filters.control?.includes(c.ctrl)}
                  isExcluded={excludes.control?.includes(c.ctrl)}
          ...
          <table className="w-full text-[11px] border-collapse">
            ...
            <tbody>
              {topControlsByCount.slice(0, 5).map((c, i) => {
                ...
                return (
                  <tr key={c.ctrl} className="hover:bg-[#f0f2f4] dark:hover:bg-[#21262d]">
                    ...
                    <td className="py-1 px-2 border-b border-[#f0f2f4] dark:border-[#21262d]">
                      <InlineFilter field="control" value={c.ctrl}
                        ...
                        <span className="text-[#e8681a]">{c.ctrl}</span>
                      </InlineFilter>
                    </td>
